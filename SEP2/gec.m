...
if(linha(1,1:4)=='DCIR')
  ...
  while(linha(1,1:4)~='####')
    ...
    elseif(linha(1,79)=='L')          % Para a barra desligada valor 1
...
%Montando o vetor com os valores de DeltaP e DeltaQ para usar no Newton Raphson
...
while abs(max(DeltaPQ))>tolerancia
    ...
    % Montando a jacobiana e Calculando H
    ...
    % Antes de montar a Jacobiana deve tratar das matrizes para que elas Recebam os valores corretos
...
fid = fopen('Resultados_sistema12B_EC1.txt','wt'); % Exporta os resultados para um .txt

fprintf(fid,'%s\n\n','------------------------------------------------------------------------------------------------------');
fprintf(fid,'%s\n','                             UNIVERSIDADE FEDERAL DE SAO JOAO DEL REI');
fprintf(fid,'%s\n\n','                          DISCIPLINA DE SISTEMAS ELETRICOS DE POTENCIA II');
fprintf(fid,'%s\n\n','------------------------------------------------------------------------------------------------------');
fprintf(fid,'%s\n','                     Grupo: Gabriel Luis de Sousa            Matricula: 180950030');
fprintf(fid,'%s\n\n','------------------------------------------------------------------------------------------------------');
...
fprintf(fid,'%3.1d %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f \n',DBAR(i,1),DBAR(i,6),DBAR(i,7),Pesp(i)*Int,Qesp(i)*Int,Pger(i)*Int,Qger(i)*Int,Sger(i)*Int,DBAR(i,2)*Int,DBAR(i,3)*Int);

end

fprintf(fid,'%s\n','------------------------------------------------------------------------------------------------------')
fprintf(fid,'Total:                                     %10.4f %9.4f %9.4f %9.4f %9.4f \n',sum(Pger*Int),sum(Qger*Int),sum(Sger*Int),sum(DBAR(:,2)*Int),sum(DBAR(:,3)*Int));
fprintf(fid,'%s\n\n','------------------------------------------------------------------------------------------------------');
...
fprintf(fid,'%3.1d %3.1d %9.4f %9.4f %9.4f  %3.1d %3.1d %9.4f %9.4f %9.4f %9.4f %11.4f\n',DCIR(t,1),DCIR(t,2),Pkm(t)*Int,Qkm(t)*Int,Skm(t)*Int,DCIR(t,2),DCIR(t,1),Pmk(t)*Int,Qmk(t)*Int,Smk(t)*Int,PerdasP(t)*Int,PerdasQ(t)*Int);

end

fprintf(fid,'%s\n','------------------------------------------------------------------------------------------------------');
fprintf(fid,'Total de Perdas:                                                             %9.4f   %9.4f\n',PerdasPtot*Int,PerdasQtot*Int);
fprintf(fid,'%s\n\n','------------------------------------------------------------------------------------------------------');
