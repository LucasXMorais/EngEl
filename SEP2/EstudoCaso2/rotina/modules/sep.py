...
def matrizAdmitancia(sistema: sistema.Sistema) -> np.ndarray:
...
def jacobian(sistema: sistema.Sistema) -> np.ndarray :

    nbarras = len(sistema.angulos)
    G = np.copy(sistema.matrizY.real)
    B = np.copy(sistema.matrizY.imag)
    angulos = np.copy(sistema.angulos)
    tensoes = np.copy(sistema.tensoes)
    dbarras = sistema.dbarras
...
def calcularFluxoBarras(sistema: sistema.Sistema):

    angulos = np.copy(sistema.angulos)
    tensoes = np.copy(sistema.tensoes)
    ncirc = len(sistema.dcircuitos)
    ...
    dcircuitos = np.copy(sistema.dcircuitos)
    ...
    sistema.fluxoPkm = fluxoPkm
    sistema.fluxoPmk = fluxoPmk
    sistema.fluxoQkm = fluxoQkm
    sistema.fluxoQmk = fluxoQmk
    sistema.fluxoSkm = fluxoSkm
    sistema.fluxoSmk = fluxoSmk
    # Fim calcularFluxoBarras

# Calculando as perdas no sistema
def calcularPerdas(sistema):
    fluxoPkm = np.copy(sistema.fluxoPkm)
    fluxoPmk = np.copy(sistema.fluxoPmk)
    fluxoQkm = np.copy(sistema.fluxoQkm)
    fluxoQmk = np.copy(sistema.fluxoQmk)
    ...
    sistema.perdasP = perdasP
    sistema.perdasQ = perdasQ
    sistema.perdasAtivasTotais = perdasAtivasTotais
    sistema.perdasReativasTotais = perdasReativasTotais
    # Fim calcularPerdas
...
def potencias(sistema: sistema.Sistema):

    nbarras = len(sistema.angulos)
    pCalc = np.copy(sistema.pCalc)
    qCalc = np.copy(sistema.qCalc)
    pG = np.zeros((nbarras,1))
    qG = np.zeros((nbarras,1))
    sG = np.zeros((nbarras,1))
    dbarras = np.copy(sistema.dbarras)
...
# def calcularFluxoKM(sistema: sistema.Sistema) -> (np.ndarray, np.ndarray):
def calcularFluxoKM(sistema: sistema.Sistema):
    G = np.copy(sistema.matrizY.real)
    B = np.copy(sistema.matrizY.imag)
    ativasKCalculada = np.zeros(np.shape(sistema.angulos))
    reativasKCalculada = np.zeros(np.shape(sistema.angulos))
    lenTensoes = len(sistema.tensoes)

    for k in range(lenTensoes):
        for m in range(lenTensoes):
            Gkm = G[k][m]
            Bkm = B[k][m]
            angulokm = sistema.angulos[k] - sistema.angulos[m]
            tensaokm = sistema.tensoes[k] * sistema.tensoes[m]
    ...
    sistema.pCalc = ativasKCalculada
    sistema.qCalc = reativasKCalculada
    # return (ativasKCalculada, reativasKCalculada)
    # Fim calcularFluxoKM


# Calculando os angulos e tensoes do problema de fluxo
def calcularAngulosTensoes(sistema: sistema.Sistema, silent: bool=False):
    ...
    nbarras = len(sistema.dbarras)
    sistema.angulos = np.zeros((nbarras,1))
    sistema.tensoes = np.ones((nbarras,1))
    for barra, v in zip(sistema.dbarras, sistema.tensoes):
    ...
    calcularFluxoKM(sistema)
    ativasKCalculada = np.copy(sistema.pCalc)
    reativasKCalculada = np.copy(sistema.qCalc)
    ...
    while iter <= maxIter :
        ...
        c = 0
        for a, t in zip(sistema.angulos, sistema.tensoes):
            a += deltaX[c]
            t += deltaX[c+len(sistema.angulos)]
            c+=1
        ...
        for barra, dp, dq in zip(sistema.dbarras, deltaPks, deltaQks):
            if barra['TIPO'] != 'PQ':
                dq[0] = 0
            if barra['TIPO'] == 'SW':
                dp[0] = 0
    ...
    sistema.angulos = [a[0] for a in sistema.angulos]
    sistema.angulosGrau = [(a*180/np.pi) for a in sistema.angulos]
    sistema.tensoes = [t[0] for t in sistema.tensoes]
    # Fim calcularAngulosTensoes
