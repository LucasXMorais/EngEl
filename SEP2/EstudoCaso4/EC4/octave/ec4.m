...
nbarras = size(BARRA,2); %Número de barras
ncircuitos = size(BDE,2); %Número de circuitos
b_carga=find(PD!=0); %Barras com carga
ncarga=size(b_carga,2);
ngerador=size(find(TIPO==2),2)+size(find(TIPO==1),2); %Número de geradores
b_ger=find(TIPO!=3);
nvariaveis=ngerador+nbarras+ncarga-1; %Número de variáveis do problema
nigualdade=nbarras; %Número de restrições de igualdade
nd_geracao=2*ngerador; %Número de restrições de desigualdade (Limite de geração)
nd_fluxo=2*ncircuitos; %Número de restrições de desigualdade (Limite de capacidade de fluxo)
...
for ic = 1:ncircuitos
    k = BDE(ic); % Barra na posição k
    m = BPARA(ic); % Barra na posição m
    % if para pegar somente as barras ligadas
    if LIG_DESL(ic)==1

        ykm = 1/(REAT(ic)*i); % Admitância com resistÊncia
        ybus(k,k) = ybus(k,k) + ykm; % Elemento da posição k,k
        ybus(k,m) = -ykm + ybus(k,m); % Elemeneto da posição k,m
        ybus(m,k) = -ykm + ybus(m,k); % Elemento da posição m,k
        ybus(m,m) = ybus(m,m) + ykm ; % Elemento da posição m,m
    endif
end
